...
* Close and create log	
	capture log close
	log using "S:\Data\_Users\Julia_S\project_s\RTA_data.log", text replace
...
************************* Generate variables **********************************
* Open the database 
	import delimited "gravity_final_OECD_ASIA.csv", clear
...
* Export estimates in Excel format
		esttab ols ppml ppml_intra ppml_intra_cfe ppml_lead ppml_lags using "Data_RTAsEffects_OECD_ASIA_2.csv", append title(Estimating the Effects of Regional Trade Agreements) mtitles(OLS PPML INTRA PAIRFE LEAD PHSNG) b(3) se(3) scalars(N r2) star(+ 0.10 * .05 ** .01) drop(EXPORTER_TIME_FE* IMPORTER_TIME_FE*) staraux nogaps

		
* Save the data used for this application 
	save "data_RTA_OECD_ASIA.dta", replace
*/